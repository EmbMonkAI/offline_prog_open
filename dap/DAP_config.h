...
#define CPU_CLOCK               84000000        ///< Specifies the CPU Clock in Hz (本工程 STM32F401 @ 84MHz)
...
#define SWD_GPIO   	GPIOB
#define PIN_SWCLK  	GPIO_PIN_6    /* OUT_SWCLK, PB6（PCB010-V3.0） */
#define PIN_SWDIO  	GPIO_PIN_7    /* OUT_SWDIO, PB7（PCB010-V3.0） */
#define PIN_nRESET  GPIO_PIN_5    /* OUT_RESET, PB5（PCB010-V3.0，低有效） */
...
static void PORT_SWD_SETUP(void)
{
  ...
  /* GPIO Ports Clock Enable */
	SWD_GPIO->BSRR = PIN_SWCLK | PIN_SWDIO | PIN_nRESET;   /* SWCLK/SWDIO/nRESET 默认高 */

  GPIO_InitStruct.Pin = PIN_SWCLK | PIN_SWDIO | PIN_nRESET;
...
static __inline void PIN_SWCLK_TCK_CLR(void)
{
	...
	#else
		SWD_GPIO->BSRR = (uint32_t)(PIN_SWCLK <<16);
	#endif
}
...
static __inline void PIN_SWDIO_TMS_CLR(void)
{
	...
	#else
		SWD_GPIO->BSRR = (uint32_t)(PIN_SWDIO <<16);
	#endif	
}
...
// nRESET Pin I/O------------------------------------------  目标硬复位 nRESET, PB7(低有效)
static __inline uint32_t PIN_nRESET_IN(void)
{
    return (SWD_GPIO->IDR & PIN_nRESET) ? 1 : 0;
}

static __inline void PIN_nRESET_OUT(uint32_t bit)
{
    if (bit & 1) SWD_GPIO->BSRR = PIN_nRESET;           /* 1=释放复位(高) */
    else         SWD_GPIO->BSRR = (PIN_nRESET << 16);   /* 0=复位(低) */
}
