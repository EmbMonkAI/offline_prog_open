...
// Write 16-bit halfword to target memory.
uint8_t swd_write_halfword(uint32_t addr, uint16_t val)
{
    uint32_t tmp;
    ...
    /* 16-bit lane steering: place data in the low/high half of DRW by addr[1] */
    tmp = (uint32_t)val << ((addr & 0x02u) << 3);
